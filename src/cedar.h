...
//  $Id: cedar.h 1739 2014-01-28 15:09:06Z ynaga $
...
namespace cedar {
  ...
  public:
    ...
    da () : tracking_node (), _array (0), _ninfo (0), _block (0), _bheadF (0), _bheadC (0), _bheadO (0), _capacity (0), _size (0), _no_delete (false)
    { _initialize (); }
    ~da () { clear (false); }
    ...
    value_type& update (const char* key, size_t& from, size_t& pos, size_t len, value_type val = value_type (0)) {
      ...
      for (const uchar* const key_ = reinterpret_cast <const uchar*> (key);
        ...
        if (val_ >= 0 && val_ != CEDAR_VALUE_LIMIT)
            { const int to = _follow (from, 0); _array[to].value = val_; }
#endif
        from = static_cast <size_t> (_follow (from, key_[pos]));
      }
#ifdef USE_REDUCED_TRIE
      const int to = _array[from].value >= 0 ? static_cast <int> (from) : _follow (from, 0);
    ...
    int next (size_t& from, size_t& len, const size_t root = 0) {
      ...
      return c ? begin (from = _array[from].base () ^ c, ++len) : CEDAR_NO_PATH;
    }
  ...
  private:
    ...
    // follow/create edge
    int _follow (size_t& from, const uchar& label) {
      int to = 0;
    ...
    int _find (const char* key, size_t& from, size_t& pos, const size_t len) const {
      ...
      if (_array[from].value >= 0) // get value from leaf
        return pos == len ? _array[from].value : CEDAR_NO_PATH;
#endif
