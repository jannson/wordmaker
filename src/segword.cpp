...
struct WordInfo {
    WordInfo(): freq(0), freq_log(FREQ_LOG_MIN), freq_test(0.0) {}
    bool calc_is_keep() {
    ...
    float calc_entropy(map<string, uint32_t>& trim) {
        if (trim.size() == 0) {
			...
			return static_cast<float>(freq)/10.0;
        }
        uint32_t trim_sum = 0;
...
typedef __gnu_cxx::hash_map<string, int> WordValidMap;
...
double get_max_f(string& word, uint32_t total_freq, WordInfoMap& wordinfo_map, FILE* fd) 
{
    WordInfo& wordinfo = wordinfo_map.find(word)->second;
	double max_ff = wordinfo.freq_log;

	if(max_ff > FREQ_LOG_MIN){
		return max_ff;
	}
	...
	if (word.size() == 2) {
		//Only one word
        f = static_cast<double>(wordinfo.freq)/total_freq;
		max_ff = log(f);
		fprintf(fd, "word:%s f:%lf total:%d max_ff:%lf\n"
			, word.c_str(), f, total_freq, max_ff);
	}
	else {
        //double f = static_cast<double>(wordinfo.freq)/total_freq;
        //wordinfo.this_log = 0.0-f*log(f);

		for (string::iterator ic = word.begin() + 2; ic <= word.end() - 2; ic += 2) {
			string temp_left(word.begin(), ic);
			double left_f = get_max_f(temp_left, total_freq, wordinfo_map, fd);
            WordInfo& word_left = wordinfo_map.find(temp_left)->second;
            f = static_cast<double>(word_left.freq)/total_freq;
            double left_f2 = log(f);

			string temp_right(ic, word.end());
			double right_f = get_max_f(temp_right, total_freq, wordinfo_map, fd);
            WordInfo& word_right = wordinfo_map.find(temp_right)->second;
            f = static_cast<double>(word_right.freq)/total_freq;
            double right_f2 =log(f);

			if((left_f + right_f) > max_ff) {
				max_ff = (left_f + right_f);
			}
            if((left_f2 + right_f2) > max_ff) {
                max_ff = (left_f2 + right_f2);
            }
			
			fprintf(fd, "word:%s left:%s right:%s=(%lf,%lf) max_ff:%lf\n"
					, word.c_str(), temp_left.c_str(), temp_right.c_str()
					, (left_f+right_f), (left_f2+right_f2), (max_ff));
		}
	}
	
	wordinfo.freq_log = max_ff;
	return max_ff;
}
double get_max_f(string& word, uint32_t total_freq, WordInfoMap& wordinfo_map) 
{
    WordInfo& wordinfo = wordinfo_map.find(word)->second;
	double max_ff = wordinfo.freq_log;
	...
	if (word.size() == 2) {
		...
		for (string::iterator ic = word.begin() + 2; ic <= word.end() - 2; ic += 2) {
			string temp_left(word.begin(), ic);
			double left_f = get_max_f(temp_left, total_freq, wordinfo_map);
            WordInfo& word_left = wordinfo_map.find(temp_left)->second;
            f = static_cast<double>(word_left.freq)/total_freq;
            double left_f2 = log(f);

			string temp_right(ic, word.end());
			double right_f = get_max_f(temp_right, total_freq, wordinfo_map);
            WordInfo& word_right = wordinfo_map.find(temp_right)->second;
            f = static_cast<double>(word_right.freq)/total_freq;
            double right_f2 = log(f);
...
int gen_cad_words(hash_set<string>* cad_words_set, WordInfoMap& wordinfo_map)
{
    ...
    fprintf(stdout, "%d %d %d\n", total_freq, wordinfo_map.size(), max_freq);
#endif
	total_freq = wordinfo_map.size()/2;
    for (WordInfoMap::iterator it = wordinfo_map.begin(); it != wordinfo_map.end(); ++it) {
...
#if 1
        //uint32_t max_ff = 0;
        double max_ff = FREQ_LOG_MIN;
        for (string::iterator ic = word.begin() + 2; ic <= word.end() - 2; ic += 2) {
            string temp_left(word.begin(), ic);
            uint32_t left_f = wordinfo_map.find(temp_left)->second.freq;
            string temp_right(ic, word.end());
            uint32_t right_f = wordinfo_map.find(temp_right)->second.freq;
            double left = log(static_cast<double>(left_f)/total_freq);
            double right = log(static_cast<double>(right_f)/total_freq);
            max_ff = max(left + right, max_ff);
            //max_ff = max(left_f * right_f, max_ff);
        }
		wordinfo.freq_test = log(static_cast<double>(wordinfo.freq)/total_freq);
        wordinfo.freq_test -= max_ff;
        int weight = (word.size()-2)*0.5;
		wordinfo.freq_test -= 0.8*(weight-1);
        //if (static_cast<double>(total_freq) * wordinfo.freq / max_ff > 100.0) {
        if(wordinfo.freq_test > 1.6) {
#endif
#if 0
        if(word.size() > 6 && !is_print){
            if(skip > 0)
                skip--;
            else
            {
                FILE* fd = fopen("test_out.txt", "w");
                double max_ff_log = get_max_f(word, total_freq, wordinfo_map, fd);
                double this_ff_log = static_cast<double>(wordinfo.freq)/total_freq;
                this_ff_log = log(this_ff_log);
                fprintf(fd, "this_log %f\n", this_ff_log);
                fclose(fd);
                is_print = true;
            }
        }

		double max_ff = get_max_f(word, total_freq, wordinfo_map);
		double this_ff = static_cast<double>(wordinfo.freq)/total_freq;
        this_ff = log(this_ff);
        double weight = (word.size()-2)/2.0;
		wordinfo.freq_test = (this_ff-max_ff)/weight;
		if(wordinfo.freq_test > 1.6) {
#endif
...
int trim_entropy_filter(list<string>& keep_words, hash_set<string>& cad_words_set, WordInfoMap& wordinfo_map)
{
	list<string> tmp_lists[WORD_LEN/2];
    ...
    for (hash_set<string>::iterator it = cad_words_set.begin(); it != cad_words_set.end(); ++it) {
        ...
        if (it_map->first.size() <= WORD_LEN-2 && it_map->second.calc_is_keep()) {
		//if (1) {
			tmp_lists[it_map->first.size()/2-2].push_back(*it);
	...
	for(int i = 0; i < WORD_LEN/2-1; i++) {
		tmp_len += tmp_lists[i].size();
		printf("%d list len:%d len2:%d len3:%d\n", i, tmp_lists[i].size(), keep_words.size(), tmp_len);
		keep_words.splice(keep_words.end(), tmp_lists[i]);
	}
	printf("hear %d length:%d \n", __LINE__, keep_words.size());

    return 0;
}

int dump_to_file(list<string>& keep_words, const char* ofile_name, WordInfoMap& wordinfo_map)
{
	int total_sum = 0;
    for (list<string>::iterator it = keep_words.begin(); it != keep_words.end(); ++it) {
        WordInfo& wordinfo = wordinfo_map.find(*it)->second;
		total_sum += wordinfo.freq;
	}
    ...
    for (list<string>::iterator it = keep_words.begin(); it != keep_words.end(); ++it) {
        ...
        fprintf(fd, "%s\t%d\t%lf\t%f\t%f\n"
                , it->c_str(), wordinfo.freq, wordinfo.freq_test
				, wordinfo.calc_entropy(wordinfo.left_trim)
				, wordinfo.calc_entropy(wordinfo.right_trim));
    }
    return 0;
}

bool sort_len_f(const string& s1, const string& s2)
{
